...
def inilization(parameters, actuators, policy, context):
    return [ActuatorPlan([random.randint(0,1) for k in range(parameters.maxNumActuators)],0,0)   for i in range(parameters.maxPopulation)], \
        ...
        if  random.random() <= parameters.crossoverRate:
            for k in range(parameters.maxNumActuators):
                ...
                if  random.random() <= parameters.mutateRate:
                    actuatorPlanNew[k] = populations[sample[2]].actuatorPlan[k]
