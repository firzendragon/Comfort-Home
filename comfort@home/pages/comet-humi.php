...
$sql = "select value from data where node_id = '$node_id' and sensor_name = 'humidity' order by context_id desc limit 1";
$result = mysql_query($sql, $link) or die("EXECUTE SQL FAILED!!");
$row = mysql_fetch_assoc($result) or die("READ DATA FAILED");
echo "<font size=50 face='Impact'>".intval($row["value"])."</font> <font style='font-size:22px;'>%</font>";
